...
def load_split(d,n):
    df=pd.read_csv(Path(d)/f"{n}_instances.csv"); return df["sequence"].astype(str).tolist(), df["y"].to_numpy()
def load_pairs(d,n):
    npz=np.load(Path(d)/f"{n}_pairs.npz"); return npz["V"].astype(np.int64), npz["U"].astype(np.int64)
...
class PairwiseProbitLikelihood(gpytorch.likelihoods.Likelihood):
    ...
    def __init__(self, init_sigma_obs=0.1):
        super().__init__(); self.log_sigma_obs=torch.nn.Parameter(torch.log(torch.tensor(init_sigma_obs)))
    @property
    def sigma_obs(self): return torch.clamp(self.log_sigma_obs.exp(), min=1e-5)
...
class VariationalInstanceModel(gpytorch.models.ApproximateGP):
    ...
    def __init__(self, inducing_points, base_kernel, init_sigma_f=0.1):
        ...
        variational_strategy = gpytorch.variational.VariationalStrategy(
            self, inducing_points, variational_dist, learn_inducing_locations=False,
        )
...
def main():
    ...
    ap.add_argument("--kernel", default="jenga", choices=["rbf","jenga"])
    ap.add_argument("--cpu", action="store_true")

    # Evidence-maximization run control
    ap.add_argument("--steps",type=int,default=5000)
    ap.add_argument("--lr",type=float,default=1.e-2)
    ap.add_argument("--seed",type=int, default=0)

    # Likelihood/prior noise
    ap.add_argument("--init_sigma_f",type=float,default=0.1, help="Init latent noise on f")
    ap.add_argument("--init_sigma_obs",type=float,default=0.1, help="Init probit observation noise")
    ...
    ap.add_argument("--n_mc", type=int,default=16, help="MC samples for E_q[log p(y|f)] in ELBO")

    # Pair subsetting / minibatching
    ap.add_argument("--max_train_pairs", type=int, default=-1,
                    help="Cap number of training pairs (subset uniformly at random). Use 0 or -1 for all.")
    ap.add_argument("--pair_batch_size", type=int, default=2**10,
    ...
    if args.max_train_pairs and args.max_train_pairs > 0 and len(V_train_np) > args.max_train_pairs:
        ...
        V_train_np, U_train_np = V_train_np[sel], U_train_np[sel]
    ...
    def resample_subset():
        ...
        V_sub = V_pool[perm]; U_sub = U_pool[perm]; num_pairs_sub = V_sub.numel(); perm_ptr = 0

    V_sub = U_sub = None; num_pairs_sub = 0; perm_ptr = 0
    resample_subset()
    ...
    for t in range(args.steps):
        ...
        if perm_ptr + bsz > num_pairs_sub:
            perm_ptr = 0
        Vb = V_sub[perm_ptr:perm_ptr+bsz]; Ub = U_sub[perm_ptr:perm_ptr+bsz]; perm_ptr += bsz
